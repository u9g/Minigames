...
class MatchingGame private constructor(private val player: Player) : Game {
    ...
    override fun start() {
        ...
        showInfoForSeconds(GameInfo.name(), GameInfo.info(), player)
            .thenAccept { it.ifFinished { startGame() } }
            .whenComplete { _, err -> err?.sendToOps() }
    }

    companion object {
        val GameInfo = object : GameInfo {
            override fun name() = "Matching"

            override fun info() = listOf(
